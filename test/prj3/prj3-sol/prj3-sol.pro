...
dept_employees(Employees, Dept, DeptEmployees) :-
  \+memberchk(Dept, [cs,ece,me]).
  dept_employees([],_, []).

dept_employees([X|Y], Dept, DeptEmployees):-
  X = employees(_,_,Dept,_), 
  DeptEmployees = [X|R1],
  dept_employees(Y,Dept, DeptEmployees).

dept_employees([_|Y], Dept,DeptEmployees):-
  dept_employees(Y, Dept, DeptEmployees).
...
employees_salary_sum(Employees, Sum) :- 
  ...
  employees_salary_sum([X|Y], Acc):-
  X = employee(_,_,_,Salary),
  Acc is Acc + Salary,
  employees(Y,Acc).
